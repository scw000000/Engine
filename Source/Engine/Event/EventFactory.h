...
class ScriptEventFactory : public ENG_Noncopyable
   {
   public:
      static ScriptEventFactory& GetSingleton( void );
      ...
      static IScriptEvent* CreateScriptEvent( EventType type );
      static IEvent* CreateEvent( EventType type );

   private:
      ScriptEventFactory( void );

      GenericObjectFactory< IScriptEvent, EventType> m_ScriptEventFactory;
      GenericObjectFactory< IEvent, EventType> m_EventFactory;
   };

template <typename T> void ScriptEventFactory::RegisterScriptEventCreation( EventType type ) 
   { 
   ENG_ASSERT( GetSingleton().m_ScriptEventFactory.Register<T>( type ) ); 
   }

template <typename T> void ScriptEventFactory::RegisterEventCreation( EventType type )
   {
   ENG_ASSERT( GetSingleton( ).m_EventFactory.Register<T>( type ) );
   }

#define REGISTER_EVENT( eventClass ) ScriptEventFactory::RegisterEventCreation<eventClass>( eventClass::s_EventType )
#define CREATE_EVENT( eventClass ) ScriptEventFactory::CreateEvent( eventClass::s_EventType )

#define REGISTER_SCRIPT_EVENT( eventClass ) \
	ScriptEventFactory::RegisterEventTypeWithScript( #eventClass, eventClass::s_EventType ); \
	ScriptEventFactory::RegisterScriptEventCreation<eventClass>( eventClass::s_EventType )

#define CREATE_SCRIPT_EVENT( eventClass ) ScriptEventFactory::CreateScriptEvent( eventClass::s_EventType )