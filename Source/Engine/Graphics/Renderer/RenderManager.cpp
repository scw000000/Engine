...
int OpenGLRenderManager::VOnRestore( Scene* pScene )
   ...
   m_BloomRenderer.OnRestoreTextures( m_SST[ SST_Lighting ], m_SST[ SST_LightThreshold ], m_SST[ SST_Bloom ] );
   m_BloomRenderer.VOnRestore( pScene );

   m_MotionBlurRenderer.OnRestoreTextures( m_SST[ SST_MRT2 ], m_SST[ SST_Bloom ], m_SST[ SST_MotionBlur ] );
...
int OpenGLRenderManager::OnRestoreTextures( void )
   ...
   glBindTexture( GL_TEXTURE_2D, m_SST[ SST_LightThreshold ] );
   glTexImage2D( GL_TEXTURE_2D, 0, GL_RGB16F, screenSize.x , screenSize.y , 0, GL_RGBA, GL_FLOAT, NULL );
   // glTexImage2D( GL_TEXTURE_2D, 0, GL_RGB16F, screenSize.x / 2, screenSize.y / 2, 0, GL_RGBA, GL_FLOAT, NULL );
   glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE );
   glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE );
   glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST );
   glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST );

   glBindTexture( GL_TEXTURE_2D, m_SST[ SST_Bloom ] );
   glTexImage2D( GL_TEXTURE_2D, 0, GL_RGB16F, screenSize.x, screenSize.y, 0, GL_RGBA, GL_FLOAT, NULL );
   glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE );
   glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE );
   glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST );
